...
for k in k_choices:
    ...
    for j in range(num_folds):
        X_train_fold = np.concatenate([X_train_folds[i] for i in range(num_folds) if i != j])
        Y_train_fold = np.concatenate([Y_train_folds[i] for i in range(num_folds) if i != j])
...
accuracies_mean = {k: np.mean(v) for k, v in k_to_accuracies.items()}
# Find the k with the highest mean accuracy
best_k = max(accuracies_mean, key=accuracies_mean.get)
print(f"\nBest k found by cross-validation: {best_k}")
# --- End of added code ---


# Now the final model is trained with the optimal best_k
knn_model = KNearestNeighbor()
knn_model.train(X_train, Y_train)
Y_test_pred = knn_model.predict(X_test, k=best_k)

# Compute and display the final accuracy
num_correct = np.sum(Y_test_pred == Y_test)
accuracy = float(num_correct) / len(Y_test) # Using len(Y_test) is better
print('Got %d / %d correct => accuracy: %f' % (num_correct, len(Y_test), accuracy))

# %%
# --- Added code to calculate and display the mean accuracies ---

# Add a separator and title for better readability
print('\n' + '-' * 40)
print('Summary of Cross-Validation Mean Accuracies')
print('-' * 40)
print('k\t | Mean Accuracy')
print('-' * 40)

# Loop through the k_to_accuracies dictionary which stores the results
for k, accuracies in sorted(k_to_accuracies.items()):
    # Use np.mean() to calculate the average of the accuracy list for the current k
    mean_accuracy = np.mean(accuracies)

    # Print the result in a clean, formatted table
    print(f'{k:d}\t | {mean_accuracy:.4f}')
