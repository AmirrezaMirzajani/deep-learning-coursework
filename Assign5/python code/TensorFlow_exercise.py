...
import tensorflow.compat.v1 as tf  # in  tensorflow_2.v some commensds changed then i add old version
import numpy as np
import math
import timeit
...
def three_layer_convnet(x, params):
    ...
    ############################################################################
    pad_1 = tf.constant([[0, 0], [2, 2], [2, 2], [0, 0]])
    x_pad = tf.pad(x, pad_1, 'constant')
    conv_1 = tf.nn.conv2d(x_pad, conv_w1, strides=[1, 1, 1, 1], padding="VALID") + conv_b1
    relu_1 = tf.nn.relu(conv_1)

    pad_2 = tf.constant([[0, 0], [1, 1], [1, 1], [0, 0]])
    relu_1_pad = tf.pad(relu_1, pad_2, 'constant')
    conv_2 = tf.nn.conv2d(relu_1_pad, conv_w2, strides=[1, 1, 1, 1], padding="VALID") + conv_b2
    relu_2 = tf.nn.relu(conv_2)

    relu_2 = flatten(relu_2)
    scores = tf.matmul(relu_2, fc_w) + fc_b
    ############################################################################
    #                              END OF YOUR CODE                            #
    ############################################################################
    return scores
...
def two_layer_fc_functional(inputs, hidden_size, num_classes):
    ...
    fc1_output = tf.keras.layers.Dense(hidden_size, activation=tf.nn.relu,
                                       kernel_initializer=initializer)(flattened_inputs)
    scores = tf.keras.layers.Dense(num_classes,
...
class ThreeLayerConvNet(tf.keras.Model):
    def __init__(self, channel_1, channel_2, num_classes):
        ...
        initializer = tf.variance_scaling_initializer(scale=2.0)
        self.conv_1 = tf.keras.layers.Conv2D(channel_1, [5, 5], [1, 1],
                                             padding='valid',
                                             kernel_initializer=initializer,
                                             activation=tf.nn.relu)
        self.conv_2 = tf.keras.layers.Conv2D(channel_2, [3, 3], [1, 1],
                                             padding='valid',
                                             kernel_initializer=initializer,
                                             activation=tf.nn.relu)
...
def model_init_fn(inputs, is_training):
    ...
    layers = [
        ...
        tf.keras.layers.Conv2D(channel_3, [3, 3], [1, 1], padding='same',
                               use_bias=True,
                               kernel_initializer=initializer,
                               bias_initializer='zeros',
                               activation=tf.nn.relu),
