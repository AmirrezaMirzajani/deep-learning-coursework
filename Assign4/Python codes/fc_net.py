...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(self.num_layers):
            ...
            self.params[f'b{i + 1}'] = np.zeros(dim_out)
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers):
            ...
            if self.normalization == 'batchnorm':
                ...
                current_input, caches[i] = affine_bn_relu_forward(current_input, W, b, gamma, beta, bn_param)
            else:  # also handles layernorm and None
                current_input, caches[i] = affine_relu_forward(current_input, W, b)

            if self.use_dropout:
                current_input, dropout_caches[i] = dropout_forward(current_input, self.dropout_param)

        # Forward pass for the last layer (output layer)
        W_last = self.params[f'W{self.num_layers}']
        b_last = self.params[f'b{self.num_layers}']
        scores, caches[self.num_layers] = affine_forward(current_input, W_last, b_last)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        for i in range(1, self.num_layers + 1):
            W = self.params[f'W{i}']
            total_reg_loss += 0.5 * self.reg * np.sum(W * W)
        loss += total_reg_loss
...
# Helper layers for batch normalization
